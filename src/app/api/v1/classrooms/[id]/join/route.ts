...
export async function POST(
  ...
  try {
    ...
    let classroom = classroomDb.getClassroom(id);
    if (!classroom && joinCode) {
      classroom = classroomDb.getClassroomByJoinCode(joinCode);
    }
    ...
    if (classroom.status === 'archived') {
      return NextResponse.json({ error: 'This classroom has been archived and is not accepting new members.' }, { status: 400 });
    }
    ...
    if (joinCode && classroom.joinCode.toUpperCase() !== joinCode.toUpperCase().trim()) {
      return NextResponse.json({ error: 'Invalid class code. Please check with your instructor.' }, { status: 400 });
    }
